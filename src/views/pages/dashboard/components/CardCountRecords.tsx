                    ...
                    return (
                        <Grid key={record} item md={3} sm={6} xs={12} >
                            <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                                <Avatar variant='rounded' sx={{ width: 50, height: 50, borderRadius: "50%", backgroundColor: hexToRGBA((mapRecord as any)[record]?.theme, 0.16) }}>
                                    <Icon icon={(mapRecord as any)[record]?.icon} fontSize={30} color={(mapRecord as any)[record].theme} />

                                </Avatar>
                                <Box>
                                    <Typography sx={{ fontSize: "26px" }}>{record === "revenue" ? `${formatNumberToLocal(data[record])} VND` : data[record]}</Typography>
