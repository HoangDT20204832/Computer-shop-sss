        ...
        <Card sx={{ ...sx,backgroundColor: hexToRGBA((mapProductStatus as any)[status]?.themeColor, 0.7) }}>
            <CardContent sx={{ gap: 3, display: 'flex', justifyContent: 'space-between' }}>
                <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                    <Typography sx={{ mb: 1, color: theme.palette.customColors.lightPaperBg }}>{(mapProductStatus as any)[status]?.title}</Typography>
                    <Box sx={{ mb: 1, columnGap: 1.5, display: 'flex', flexWrap: 'wrap', alignItems: 'center' }}>
                        <Typography variant='h4' sx={{ color: theme.palette.customColors.lightPaperBg, fontWeight: "bold" }}>{(mapProductStatus as any)[status]?.count}</Typography>
                    </Box>
                </Box>
